...
lint/isort: ## check style with flake8
	isort --check psy_view tests
lint/flake8: ## check style with flake8
	flake8 psy_view tests
lint/black: ## check style with black
	black --check psy_view tests
	blackdoc --check psy_view tests
...
formatting:
	isort psy_view tests
	black psy_view tests
	blackdoc psy_view tests

quick-test: ## run tests quickly with the default Python
	python -m pytest

pipenv-test: ## run tox
	pipenv run isort --check psy_view
	pipenv run black --line-length 79 --check psy_view
	pipenv run flake8 psy_view
	pipenv run pytest -v --cov=psy_view -x
...
coverage: ## check code coverage quickly with the default Python
	python -m pytest --cov psy_view --cov-report=html
